...
## DDR-016 — T13 "opsiyon düşerse haber ver": bildirim `birim.durum` trigger'ında (tüm release yollarında evrensel)
Date: 2026-08-20 · Status: Accepted (MODE B) · Class: Platform invariant (notify model) · bkz. INV-BEKLE-001, INV-CRON-001, DDR-015
**Evrim (iki adım):**
- **v1 (`c17c3eb`, migration `db/2026-08-20_opsiyon-bekleyen.sql`):** bildirim+temizlik `opsiyon_serbest_birak`'a additive CTE olarak eklendi. MODE B safe bulundu ANCAK P2 kapsam sınırı: notify YALNIZ cron-expiry yolunda; gönüllü bırakma/vazgeçme/manuel release bildirmiyor, `satis_beklemede` takibi orphan kalıyordu (DEBT-011). Buton vaadi en olası "vazgeçildi" yolunu ıskalıyordu.
- **v2 (`8ab1438`, migration `db/2026-08-20b_bekleyen-tum-yollar.sql`) — SÜPERSEDES v1, KANONİK:** notify+cleanup `opsiyon_serbest_birak`'tan ALINDI → fonksiyon **pür-N10 expiry'ye geri döndü** (yalnız `del`+`ev` CTE; INV-CRON-001/002 + B3/INV-STATE-001 saf, canlı `pg_get_functiondef` doğrulandı). Mantık `birim.durum` trigger'ına taşındı: `birim_bekleyen_islet_trg` AFTER UPDATE OF durum ON birim.
**v2 kanonik mekanizma (neden evrensel — MODE B doğrulandı):**
- `opsiyon_birim_trg` = AFTER INSERT/**DELETE**/UPDATE ON opsiyon → `opsiyon_birim_senkron`. DELETE branch: başka aktif opsiyon yoksa `update birim set durum='musait'`; UPDATE→'satildi' de birim'e yansır. Yani opsiyon'un HER bitiş yolu (cron `opsiyon_serbest_birak`, `opsiyonBirakSessiz` `actions.ts:335`, `vazgecildi` `:423`, `opsiyon_serbest_birak_manuel`, satış) birim.durum'u değiştirir → trigger evrensel yakalar.
- `birim_bekleyen_islet`: `→musait` (old in opsiyonlu/satis_beklemede) = takipçilere bildir + tüm takipleri sil (tek-seferlik); `→satildi/kiralandi` = takipleri sil (orphan yok, bildirim yok). `satis_beklemede→musait` (satış iptali) artık DA kapsanır (v1'de kaçıyordu).
- **Idempotency v1'den GÜÇLÜ:** `old.durum is distinct from 'musait'` guard → birim.durum iki kez 'musait' yazılsa bile tek bildirim (concurrent cron+failsafe: opsiyon del row-lock → birim tek transition). İki-seviye trigger zinciri (opsiyon→birim→bekleyen) recursion'suz sonlanır (birim trigger birim'i yeniden yazmaz).
- **Satıcı-hariç kuralı KALDIRILDI, gereksiz:** buton yalnız `!benimOpsiyon` iken gösterilir (`DaireModal.tsx:332`) → satıcı kendi tuttuğu birimi takip edemez; v1'deki `is distinct from satici_id` filtresine ihtiyaç yok.
- **RLS/leak:** SECURITY DEFINER insert; gövde birim/fiyat SIZDIRMAZ, link `/danisman/proje/{proje_id}` RLS-gated (revoke edilmiş takipçi yalnız "müsait oldu" öğrenir, tıklayınca RLS-deny).
- **Decision (v2): APPROVE** — v1'in P2/DEBT-011'i kapandı; expiry fonksiyonu N10 saflığına döndü; mekanizma daha temiz + evrensel + daha güçlü idempotent. Kalan P0/P1/P2 yok.
