...
| INV-AUDIT-001 | Her state-değiştiren admin mutasyonu `events`'e satır yazar (actor + before/after); "denetim" UI bütünlük iddiası buna bağlı | PLATFORM INVARIANT | **İHLAL:** hesap açma/rol/paket fiyatı/üretici rozeti event yazmıyor → audit E2/F1 | ✗ |
| INV-SALE-001 | Tüm "→satıldı" geçişleri tek satış-kapama yolundan (hakediş + satıcı attribution); grid'de ham `durum=satildi` yasak | PLATFORM INVARIANT | **İHLAL:** `birimDurumGuncelle(satildi)` hakediş yazmıyor → audit C1/U-04 | ✗ |
| INV-STATE-001 | Hiçbir cron path `satis_beklemede` birimi serbest bırakamaz (sözleşme aşaması korunur) | PLATFORM INVARIANT | **İHLAL:** expiry cron `satis_beklemede` opsiyonu silip birimi `musait` yapıyor → audit B3/XP-03 | ✗ |
