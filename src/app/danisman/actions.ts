...
export async function opsiyonTalepGonder(
  ...
  revalidatePath(`/danisman/proje/${p.data}`);
  revalidatePath("/danisman");
  revalidatePath("/danisman/opsiyonlarim");
...
export async function belgeYukle(formData: FormData): Promise<void> {
  ...
  if (!r.ok) redirect(`/danisman/dogrulama?hata=${encodeURIComponent(r.hata)}`);
  revalidatePath("/danisman/dogrulama");
  revalidatePath("/danisman");
  redirect(`/danisman/dogrulama?mesaj=${encodeURIComponent(r.yuklendi > 0 ? "Belgeler yüklendi — doğrulama bekleniyor" : "Dosya seçilmedi")}`);
}
...
export async function paylasimKaydet(projeId: string, birimId?: string | null): Promise<void> {
  ...
  revalidatePath("/danisman/paylastiklarim");
}
...
export async function leadDurumGuncelle(leadId: string, yeniDurum: string): Promise<{ ok: boolean }> {
  ...
  revalidatePath("/danisman/leadler");
  revalidatePath(`/danisman/leadler/${id.data}`);
...
export async function leadNotEkle(
  ...
  revalidatePath(`/danisman/leadler/${yetki.lead_id}`);
  revalidatePath("/danisman/leadler");
  return { ok: true };
}
