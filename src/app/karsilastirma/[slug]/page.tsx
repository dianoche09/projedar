...
import { KARSILASTIRMA_GOVDE } from "@/content/karsilastirma";
...
export function generateStaticParams() {
  return kategoriIcerikleri("karsilastirma").map((m) => ({ slug: m.slug }));
}
...
export async function generateMetadata({
  ...
  const meta = icerikBul("karsilastirma", slug);
  if (!meta) return {};
  return {
...
export default async function KarsilastirmaSayfa({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const { slug } = await params;
  const meta = icerikBul("karsilastirma", slug);
  const modul = KARSILASTIRMA_GOVDE[slug];
