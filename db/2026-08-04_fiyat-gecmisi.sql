...
-- Yalnız gerçek DEĞİŞİM loglanır: ilk fiyatlama (null/0→değer), fiyat silme (değer→null) ve
-- 0'dan çıkış hariç. OLD=0 kontrolü sıfıra bölmeyi de önler (yoksa 0→X güncellemesi
-- 'division by zero' fırlatır → UPDATE geri alınır → birim kalıcı fiyatlanamaz).
...
-- Sorgu deseni (raporlar/talep-radari): tip filtresi + created_at aralığı.
-- events tablosunda yalnız birim_id index'i vardı; created_at aralık sorguları seq-scan'diydi.
create index if not exists events_tip_created_idx on events (tip, created_at desc);
